...
if [ x"$SCRIPT_FORCE_UPDATE" = x1 ]; then
	...
	curl -s "http://gitlab.xta.net/internal/automation/raw/master/full.sh" >full.sh~
	if [ "$?" != "0" ]; then
...
LAST_VERSION=`curl -s "http://gitlab.xta.net/internal/automation/raw/master/versions.txt" | grep "^full.sh" | awk '{ print $2 }'`
