...
if [ x"$SCRIPT_FORCE_UPDATE" = x1 ]; then
	...
	curl -s "https://raw.githubusercontent.com/shade3d/automation/stable/M4_TARGET" >M4_TARGET~
	if [ "$?" != "0" ]; then
...
LAST_VERSION=`curl -s "https://raw.githubusercontent.com/shade3d/automation/stable/versions.txt" | grep "^M4_TARGET" | awk '{ print $2 }'`
